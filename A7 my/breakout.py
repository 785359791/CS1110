...
class Breakout(GameApp):
    ...
    LIST MORE ATTRIBUTES (AND THEIR INVARIANTS) HERE IF NECESSARY
        _lives   [int and <=3 0,,if no remaining lives,]
                remaining lives
        _time   [int >= 0, 0 if state changed]:
                frames since state  changed 
    """
    ...
    def start(self):
        ...
        #assert _state in [STATE_INACTIVE, STATE_COUNTDOWN, STATE_PAUSED, STATE_ACTIVE]
        #assert _game is Play or None 
        #assert _mssg is GLabel or None
        self._state=STATE_INACTIVE
        self._time=0
        self._game=None
        self._mssg= GLabel(text= 'Press any key to start',font_size=30,x=GAME_WIDTH/2.0,y=GAME_HEIGHT/2.0,font_name='ComicSans.ttf',)
    ...
    def update(self,dt):
        ...
        if self._state==STATE_INACTIVE:
            ...
            if self._game.getBall().bottom<=0 and self._game.getTries()>1:
                self._game.reduceTries()   
                self._state = STATE_PAUSED
            if self._game.getBall().bottom<=0 and self._game.getTries()==1:    
                self._mssg= GLabel(text= 'Sorry :( You lost \n Press any key to play again',font_size=30,font_name='ComicSans.ttf',x=GAME_WIDTH/2.0,y=GAME_HEIGHT/2.0)
                if self.checkChange():
                    self._state=STATE_NEWGAME
            if self._game.getBricks()==[]:
                self._state = STATE_COMPLETED
        elif self._state == STATE_PAUSED:
            self._mssg= GLabel(text= 'You have ' + str(self._game.getTries())+ ' try \n Press any key to restart',font_size=30,font_name='ComicSans.ttf',x=GAME_WIDTH/2.0,y=GAME_HEIGHT/2.0)
            if self.checkChange():
                self._mssg=None
                self._state = STATE_COUNTDOWN
        elif self._state == STATE_COMPLETED:
            self._mssg= GLabel(text= 'Congradulations :) You won \n Press any key to play again',font_size=30,font_name='ComicSans.ttf',x=GAME_WIDTH/2.0,y=GAME_HEIGHT/2.0)
            if self.checkChange():
                self._state = STATE_NEWGAME
    ...
    def draw(self):
        ...
        if self._state==STATE_COUNTDOWN or self._state==STATE_PAUSED:
            for brick in self._game.getBricks():
    ...
    # HELPER METHODS FOR THE STATES GO HER
    def checkChange(self):
        curr_keys = self.input.key_count
        if curr_keys > 0 and self._last == []:
            return True
        
    
    def countDown(self):
        self._mssg= GLabel(text= '3',font_size=30,font_name='ComicSans.ttf',x=GAME_WIDTH/2.0,y=GAME_HEIGHT/2.0)
        self._time=self._time+1
        if 60 <=self._time < 120:
            self._mssg= GLabel(text= '2',font_size=30,font_name='ComicSans.ttf',x=GAME_WIDTH/2.0,y=GAME_HEIGHT/2.0)
        if 120 <=self._time < 180:
            self._mssg= GLabel(text= '1',font_size=30,font_name='ComicSans.ttf',x=GAME_WIDTH/2.0,y=GAME_HEIGHT/2.0)
